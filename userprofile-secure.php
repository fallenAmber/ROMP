...
if(isset($_POST['delete'])){
	$batch = $_POST['batch'];
	$query = "DELETE FROM $batch WHERE $batch.Email = '".$_POST['dkey']."'";
	mysqli_query($connection, $query);
	echo '<br><br><br><br><h2>RECORD HAS BEEN DELETED</h2>';
}
...
if((isset($_POST['searchbyname']))){
	...
	if($_POST['person'] == "student"){
		...
		if($keyword == "ALL" || $keyword == "All" || $keyword == "all" || $keyword == $_POST['series'] || $keyword == $_POST['department'] || $keyword == NULL){
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch";
		}
		else if(strlen($keyword) == 1 && ($keyword >= 'A' && $keyword <= 'Z')){
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Fullname LIKE '".$keyword."%'";
		}
		else{
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Fullname LIKE '%".$keyword."%'";
		}
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		echo '<td>';
		echo '<form action="studentprofile-secure.php" method="post">';
		...
		echo '</form>';
		if($Emailpass == $Email || $Emailpass == "admin@localhost"){
		echo '<form action="userprofile-secure.php" method="post">';
		echo '<input type="hidden" name="dkey" value="'.$Email.'" />';
		echo '<input type="hidden" name="batch" value="'.$batch.'" />';
		echo '<input type="hidden" name="emailpass" value="'.$_SESSION['email'].'" />';
...
if((isset($_POST['searchbyname']))){
	...
	while ($row = mysqli_fetch_array($result)) {
		...
		echo '<td>';
		echo '<form action="teacherprofile-secure.php" method="post">';
		echo '<input type="hidden" name="name" value="'.$data['Fullname'].'" />';
		echo '<input type="hidden" name="emailpass" value="'.$_SESSION['email'].'" />';
		echo '<input type="submit" name="searchbyname" id="1" value="VIEW PROFILE"/>';
		echo '</form>';
...
if((isset($_POST['searchbyemail']))){
	...
	if($_POST['person'] == "student"){
		...
		if($keyword == "ALL" || $keyword == "All" || $keyword == "all" || $keyword == NULL){
			...
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Email LIKE '".$keyword."%'";
		}
		else{
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Email LIKE '%".$keyword."%'";
		}
...
if((isset($_POST['searchbyroll']))){
	...
	if($_POST['person'] == "student"){
		...
		if($keyword == "ALL" || $keyword == "All" || $keyword == "all" || $keyword == NULL){
			...
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Roll LIKE '".$keyword."%'";
		}
		else{
			$query = "SELECT Roll,Fullname,Department,Series,Email FROM $batch WHERE ".$batch.".Roll LIKE '%".$keyword."%'";
		}
